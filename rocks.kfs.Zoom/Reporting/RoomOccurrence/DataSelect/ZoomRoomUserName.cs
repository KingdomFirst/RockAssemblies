...
{
    /// <summary>
    /// A tabular report field that displays the Zoom Room User's Name of the Zoom Room associated with the RoomOccurrence.
    /// </summary>
    [Description( "Show Zoom Room User Name" )]
    [Export( typeof( DataSelectComponent ) )]
    [ExportMetadata( "ComponentName", "Select Zoom Room User Name" )]
    public class ZoomRoomUserNameSelect : DataSelectComponent
    {
        /// <summary>
        ...
        public override System.Linq.Expressions.Expression GetExpression( Rock.Data.RockContext context, System.Linq.Expressions.MemberExpression entityIdProperty, string selection )
        {
            ...
            var zoomRoomUserNameAttGuid = ZoomGuid.Attribute.ZOOM_USER_NAME.AsGuid();
            var zrUserNameValuesQuery = attributeValueService.Queryable()
                .Where( x => x.Attribute.Guid == zoomRoomUserNameAttGuid )
            ...
            var resultQuery = occurrenceService.Queryable( "Location" )
                .Select( ro => zrUserNameValuesQuery.FirstOrDefault( un => definedValueQuery.FirstOrDefault( dv => valuesQuery.FirstOrDefault( v => v.EntityId == ro.Location.Id && v.Value == dv.GuidString ).Value == dv.GuidString ).Id == un.EntityId ).Value );
